...
//            map.put("no",null);//no라는 키가 있어도 null값이 들어가면 mybatis에서는 없는 것으로 인식
            map.put("no", 10); //no라는 키가 있어야 mybatis에서 인식함
            map.put("sal", 3000);
            List<EmpVO> list = ss.selectList("emp.search_dept4", map);
//            List<EmpVO> list = ss.selectList("emp.search_dept2", map);
            //List<EmpVO> list = ss.selectList("emp.search_dept", 10);
