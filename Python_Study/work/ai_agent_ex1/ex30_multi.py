            ...
            for handoff_agent in agent.handoffs:
                if handoff_agent.name in handoff_target:
                    ...
                    new_response= cls.client.messages.create(
                        model="claude-3-5-haiku-latest",
                        max_tokens=500,
                        messages=[{"role":"user","content":f"{handoff_agent.inst}\n\n환자메시지: {message}"}]
                    )
...
async def simple_handoff_example():
    ...
    doctor1 = Agent(
        name="정형외과 전문의",
        inst="근골격계 문제(허리통증,관절염,골절 등)를 진료합니다."
    )

    # 정형외과 전문의 에이전트
    doctor2 = Agent(
        name="이비인후과 전문의",
        inst="코,귀,목의 질환을 진료합니다."
    )

    # 정형외과 전문의 에이전트
    doctor3 = Agent(
        name="내과 전문의",
        inst="내과질환(감기,소화불량,두통 등)을 진료합니다. 근골격계문제는 정형외과 의사에게 연결합니다."
    )
